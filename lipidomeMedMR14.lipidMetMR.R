...
exposureFile="lipidome.exposure.F.csv"             #脂质体的暴露数据文件
sigLipidFile="lipidome-disease.REVfilter.csv"      #脂质体过滤的结果文件
setwd("C:\\Users\\lexb\\Desktop\\lipidomeMedMR\\14.lipidMetMR")     #设置工作目录

#读取暴露数据输入文件
rt=read.csv(exposureFile, header=T, sep=",", check.names=F)

#读取脂质体过滤的结果文件
sigLipid=read.csv(sigLipidFile, header=T, sep=",", check.names=F)
...
#对脂质体进行循环
for(Lipid in unique(sigLipid$id.exposure)){
	#提取这个脂质体的暴露数据
	k=gsub("\\%|\\/|\\:", "_", Lipid)
	singleExposureFile=paste0(k, ".exposure.csv")
	exposure_set=rt[rt$id.exposure==Lipid,]
	...
	for(outcomeFile in gzFiles){
		...
		i=paste0("beta1.", Lipid, "_", outcomeName)
		#读取结局数据
